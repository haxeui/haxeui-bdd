...
class ProfileRunner {
    ...
    private function nextProfile() {
        ...
        var profile:Dynamic = _profiles[0];
        _profiles.remove(profile);
        if (profile.active != null && profile.active == false) {
            nextProfile();
            return;
        }
        ...
        context.addObject("Files", FileUtil);
    ...
    private function startProfile(profile) {
        ...
        switch (profile.type) {
            ...
            case "swf":
                ...
                InputSimulatorUtil.handleFromProcessId(processId);
            case "browser":
                profile.settings.server = replaceVars(profile.settings.server);
                ...
                _browserProcess = new Process("C:/Program Files (x86)/Mozilla Firefox/firefox.exe", ["-silent", "-new-instance", "-url", profile.path]);
